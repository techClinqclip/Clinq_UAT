...
import { X, IndianRupee, Smartphone, AlertCircle } from "lucide-react";
...
/*
  AddFundsModal — top-up flow for the creator's campaign-spend wallet.
  Same shape as the Brand version (see BrandWallet.jsx's AddFundsModal):
  it reflects whatever payment method is saved on Profile > Payments
  read-only, with a "Change" link that sends them there to update it
  instead of picking a method here.

  Props:
    isOpen, onClose
    onSuccess(amount)   — called after the (simulated) top-up completes
    savedMethod         — { method, label, detail, icon } | null
                          Build this from the creator's profile — see
                          getSavedPaymentMethodDisplay() in
                          CreatorWallet.jsx for the exact shape.
*/
        ...
        >
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-zinc-500">Top up your wallet to launch and fund your own campaigns.</p>
            </div>
            <button
              onClick={handleClose}
