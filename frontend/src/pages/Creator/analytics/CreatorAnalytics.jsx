...
/*
  Brought in line with the Brand Analytics page: paired KPI cards
  (Views + Earnings together, Unique Viewers + Avg Watch Time together
  as "Coming Soon"), and an EmptyState fallback on every section that
  didn't have one before (charts, platform breakdown, gig table, top
  gigs, top clippers) instead of silently rendering empty/blank areas.

  NOTE: GIGS_ROUTE below is a guess ("/marketplace"), matching the
  "Browse More Gigs" links used elsewhere in the creator/clipper flow.
  Point it at whatever your real gig-browsing route is if different.
*/
...
/* ---------- Small building blocks (mirrors Brand Analytics) ---------- */
...
export default function Analytics() {
  ...
  useEffect(() => {
    ...
    const loadAnalytics = async () => {
      try {
        setLoading(true);
        setError("");
        const data = await api("/api/creator/analytics/");
        if (!isMounted) return;
  ...
  const totalPayout = activePayoutData.reduce((sum, d) => sum + Number(d.payout || 0), 0) || Number(analytics?.total_earned || 0);
  ...
  const topClippers = analytics?.top_clippers || [];

  const hasGigs = topGigs.length > 0;
  ...
  const hasClippers = topClippers.length > 0;
  ...
  return (
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="mt-2 text-zinc-400">Track your gigs, earnings, and audience growth.</p>
      </div>

      {error ? <div className="rounded-2xl border border-red-500/20 bg-red-500/10 p-4 text-sm text-red-300">{error}</div> : null}

      {/* Stat cards: Total Views + Total Earned grouped on the left,
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          <div className="grid grid-cols-2 divide-x divide-white/10 rounded-3xl border border-white/10 bg-[#11111A] transition hover:border-violet-500/30">
            ...
            <StatHalf
              ...
              value={`₹${Number(analytics?.total_earned ?? 0).toLocaleString()}`}
              label="Total Earned"
            />
          </div>
      ...
      {/* Shared time filter for both charts below */}
      ...
      <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="mt-6 rounded-2xl border border-violet-500/20 bg-[#0B0B12] p-4">
            {hasViewsData ? (
              ...
              <EmptyState
                icon={LineChart}
                title="No views yet"
                description={`No view data for ${filter}. Views will show up here once your content starts getting seen.`}
        ...
        <div className="overflow-hidden rounded-3xl border border-emerald-500/10 bg-gradient-to-br from-emerald-600/10 to-transparent p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm uppercase tracking-[0.2em] text-emerald-400">Earnings Overview</p>
              <h3 className="mt-3 text-4xl font-bold text-white">₹{formatCompact(totalPayout)}</h3>
              <p className="mt-2 text-sm text-zinc-500">Earned for {filter}</p>
            </div>
          ...
          <div className="mt-6 rounded-2xl border border-emerald-500/20 bg-[#0B0B12] p-4">
            {hasPayoutData ? (
              ...
              <EmptyState
                ...
                description={`Nothing earned for ${filter}. Your earnings will show up here once your submissions get approved.`}
                ctaLabel="Browse Gigs"
                ctaTo={GIGS_ROUTE}
                compact
              />
      ...
      <section className="rounded-3xl border border-white/10 bg-[#11111A] p-8">
        ...
        {hasPlatformData ? (
          ...
          <EmptyState
            ...
            description="Once your content starts getting views across YouTube, Instagram, and more, their share of views will show up here."
            ctaLabel="Browse Gigs"
            ctaTo={GIGS_ROUTE}
          />
        )}
      ...
      <section className="rounded-3xl border border-white/10 bg-[#11111A] p-8">
        ...
        {hasGigs ? (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/10">
                  <th className="pb-4 text-left text-zinc-500">Gig</th>
              ...
              <tbody>
                {topGigs.map((gig) => (
                  <tr key={gig.id} className="border-b border-white/5">
                    <td className="py-5 text-white">{gig.name}</td>
                    <td className="py-5 text-white">{gig.views}</td>
                    <td className="py-5 text-white">{gig.submissions}</td>
                    <td className="py-5 font-semibold text-violet-400">{gig.payout}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <EmptyState
            icon={ClipboardList}
            title="No gigs yet"
            description="Join your first gig to start collecting views, submissions, and payouts here."
            ctaLabel="Browse Gigs"
            ctaTo={GIGS_ROUTE}
          />
        )}
      ...
      {/* Top Gigs + Top Clippers */}
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="rounded-3xl border border-white/10 bg-[#11111A] p-8">
          <div className="flex items-center gap-2">
            <Trophy size={20} className="text-amber-400" />
            <h2 className="text-2xl font-bold text-white">Top Gigs</h2>
          </div>
          ...
          {hasGigs ? (
              ...
              {topGigs.map((gig, i) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <h3 className="truncate font-semibold text-white">{gig.name}</h3>
                    <p className="mt-1 text-violet-400">{gig.views} views</p>
                  </div>
                </Link>
              ))}
            </div>
            ...
            <EmptyState
              icon={BarChart3}
              title="No gigs to rank"
              description="Your best-performing gigs will be ranked here by views."
              ctaLabel="Browse Gigs"
              ctaTo={GIGS_ROUTE}
              compact
            />
        ...
        <section className="rounded-3xl border border-white/10 bg-[#11111A] p-8">
          ...
          {hasClippers ? (
              ...
              {topClippers.map((clipper, i) => (
                <div key={`${clipper.username}-${i}`} className="flex items-center gap-4 rounded-2xl border border-white/10 p-4">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-1 text-zinc-400">{clipper.views} views</p>
                  </div>
                  <p className="shrink-0 font-semibold text-violet-400">{clipper.earnings}</p>
                </div>
              ))}
            </div>
          ) : (
            <EmptyState
              ...
              description="Once fellow clippers start submitting content, the leaderboard will show up here."
              ctaLabel="Browse Gigs"
              ctaTo={GIGS_ROUTE}
              compact
            />
