...
function CarouselShell({ children }) {
  return (
    <div className="relative h-[360px] overflow-hidden rounded-3xl sm:h-[440px] lg:h-[470px]">
      {children}
    </div>
  );
}
...
export default function BannerCarousel({
  ...
  useEffect(() => {
    ...
    setState(([prev]) => [
      Math.min(prev, featuredCampaigns.length - 1),
      1,
    ]);
  ...
  const next = () =>
    goTo((current + 1) % featuredCampaigns.length, 1);
        ...
        <div className="flex h-full flex-col items-center justify-center gap-4 rounded-3xl border border-dashed border-white/10 bg-white/[0.02] px-6 text-center">
          ...
          <div>
            <p className="font-medium text-zinc-300">
              No campaigns to feature yet
            </p>

            <p className="mt-1 text-sm text-zinc-500">
              Check back soon — new campaigns show up here first.
            </p>
          </div>
        </div>
      </CarouselShell>
  ...
  return (
    <div
      className="relative h-[360px] overflow-hidden rounded-3xl sm:h-[440px] lg:h-[470px]"
    ...
    >
      ...
      >
        ...
        >
          <BannerSlide
            campaign={featuredCampaigns[current]}
            onJoinClick={openCampaignDetails}
          />
        </motion.div>
      </AnimatePresence>
...
function BannerCarouselSkeleton() {
  return (
    <div className="relative h-[360px] overflow-hidden rounded-3xl border border-white/10 bg-white/[0.03] sm:h-[440px] lg:h-[470px]">
      ...
      <div className="relative z-10 flex h-full items-center px-5 pb-20 pt-5 sm:px-10 sm:pb-24 sm:pt-10 lg:px-14 lg:pb-16 lg:pt-14">
        <div className="w-full max-w-xl">
          <div className="h-6 w-36 animate-pulse rounded-full bg-white/10 sm:h-8 sm:w-48" />

          <div className="mt-4 h-8 w-3/4 animate-pulse rounded-lg bg-white/10 sm:mt-6 sm:h-11 lg:mt-8 lg:h-14" />
          ...
          <div className="mt-4 flex flex-wrap gap-3 sm:mt-6 sm:gap-6 lg:mt-8 lg:gap-8">
            <div className="h-5 w-24 animate-pulse rounded bg-white/10" />
            <div className="h-5 w-28 animate-pulse rounded bg-white/10" />
            <div className="h-5 w-20 animate-pulse rounded bg-white/10" />
          </div>

          <div className="mt-5 h-11 w-36 animate-pulse rounded-xl bg-white/10 sm:mt-7 sm:h-12 sm:w-40 lg:mt-10" />
        </div>
      </div>

      <div className="absolute bottom-4 left-1/2 z-10 flex -translate-x-1/2 gap-1.5 sm:bottom-8">
        {Array.from({ length: 4 }).map((_, i) => (
          <span
            key={i}
            className="h-1.5 w-6 animate-pulse rounded-full bg-white/15"
          />
        ))}
      </div>
    </div>
